...
class AuthController {
  ...
  login = async (req, res, next) => {
    try {
      ...
      console.log('Executing query: SELECT * FROM users WHERE email = ? LIMIT 1');
      ...
      console.log('Query returned:', users ? users.length : 'null/undefined', 'rows');
      const user = users[0];
      console.log('User found:', user ? `Yes (${user.email})` : 'No');
      if (user) {
        console.log('User details:', { id: user.id, email: user.email, status: user.status, hasPassword: !!user.password });
      }
